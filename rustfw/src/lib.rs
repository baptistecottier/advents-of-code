...
macro_rules! main {
    () => {
        ...
        fn main() {            
            ...
            let puzzle_input = read_to_string(format!("../events/{}/{}/{}.input", year, day, day).trim_end()).expect("File does not exist") ;
...
pub fn screen_reader(pixels: HashSet<(usize, usize)>) -> String {
    ...
    .group_by(|(x, _)| x / 5)
    .into_iter()
    .map(|(k, v)| (k, v.map(|(x, y)| (x % 5, y)).collect::<Vec<(usize, usize)>>()))
    .map(|(_, v)| 
        match v.len() {
            9 => 
                if      v.contains(&(3, 0)) {'J'} 
            ...
            12 => 
                if     !v.contains(&(0, 0)) {'O'}
                else if v.contains(&(1, 2)) {'K'}
                else if v.contains(&(3, 5)) {'Z'}
                else if v.contains(&(3, 4)) {'U'}
                else                        {'P'}
            13 =>                            'G',
            14 => 
                if     !v.contains(&(0, 0)) {'A'}
                else if v.contains(&(1, 3)) {'R'}
                else if v.contains(&(2, 0)) {'E'}
                else                        {'H'},
            _  =>                            'B'})
    .join("")
