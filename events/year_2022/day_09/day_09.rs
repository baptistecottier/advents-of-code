...
fn preprocessing(input: &str) -> Vec<(i32, i32)> {
    input
        ...
        .fold(vec![(0, 0)], |mut head_pos, (n, (dx, dy))| {
            head_pos.append(
                &mut (1..=n)
                    .scan(head_pos.last().unwrap(), |new_pos, i| {
...
fn solver(mut path: Vec<(i32, i32)>, knots: usize) -> usize {
    (0..knots - 1).for_each(|_| {
        let mut tail_pos = (0, 0);
        path.clone().iter().enumerate().for_each(|(n, head_pos)| {
            let d =
                ((head_pos.0 - tail_pos.0).abs() > 1 || (head_pos.1 - tail_pos.1).abs() > 1) as i32;
