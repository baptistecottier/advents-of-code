    ...
    input.split("\n\n").enumerate().for_each(|(i, monkey)| {
        ...
        starting_items.append(
            &mut details[0]
                ...
                .split(' ')
                .skip(2)
                .map(|n| (n.parse::<u64>().unwrap(), i))
                .collect_vec(),
        );
        ops.append(&mut match details[1]
            .split("= old ")
            .nth(1)
            .unwrap()
            .split(' ')
...
fn solver(
    input: (Vec<(u64, usize)>, Vec<[u64; 3]>, Vec<Vec<u64>>),
    rounds: usize,
    div: u64,
) -> u64 {
    ...
    items.iter().for_each(|&(mut v, mut new_monkey)| {
        (0..rounds).for_each(|_| {
            ...
            while new_monkey >= old_monkey {
                ...
                v = ((v * (mul + sq * v) + add) / div) % (div * modulo);
