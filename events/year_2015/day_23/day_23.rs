...
fn preprocessing(puzzle_input: &str) -> Vec<(&str, i32, i32)> {
    ...
    .map(|(f, args)| 
        (f,
        args.split_once(", ").unwrap_or_else(|| (args, "0"))))
    .map(|(f, (reg, offset))| 
        (f,
        match reg {"a" => 0, "b" => 1, _ => reg.parse().unwrap()}, 
        offset.parse().unwrap()))
    .collect_vec()
    }
    ...
    *(0..)
    .fold_while((0, [reg_a, 0]), |(mut i, mut reg), _| {
        ...
        match f {
            ...
            "jie" => if reg[r as usize].is_even() {i = (i as i32 + offset - 1) as usize;},
            "jio" => if reg[r as usize] == 1 {i = (i as i32 + offset - 1) as usize;},
            _ => unreachable!(), };
        if i + 1 < program.len() {FoldWhile::Continue((i + 1, reg))} else {FoldWhile::Done((i, reg))}
        }
    )
    .into_inner()
    .1
