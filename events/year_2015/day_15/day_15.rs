    ...
    input
        ...
        .map(|v| {
            [
                ...
                v[8].parse::<i64>().unwrap(),
                v[11].parse::<i64>().unwrap(),
                v[14].parse::<i64>().unwrap(),
            ]
        })
        .collect_vec()
}
                ...
                .map(|j| {
                    ...
                    (0..100 - (i + j))
                        .map(|k| {
                            ...
                            let sugar = ingredients[3]
                                .iter()
                                .map(|elem| (100 - (i + j + k)) * elem)
                                .collect_vec();
                            let calories = sprinkles[4] + peanutbutter[4] + frosting[4] + sugar[4];
                            if check_calories && calories != 500 {
                                0
                            } else {
                                (0..4)
                                    .map(|v| {
                                        let s =
                                            sprinkles[v] + peanutbutter[v] + frosting[v] + sugar[v];
                                        if s < 0 {
                                            0
                                        } else {
                                            s
