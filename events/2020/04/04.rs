...
fn part_2(passports: Vec<HashMap<&str, &str>>) -> usize{
    ...
    .filter(|passport|
        ...
        && (   (passport["hgt"].contains("in") && (59..=76).contains(&passport["hgt"][..2].parse::<u16>().unwrap_or(0)))
            || (passport["hgt"].contains("cm") && (150..=193).contains(&passport["hgt"][..3].parse::<u16>().unwrap_or(0))))
        && (passport["hcl"].starts_with('#') && passport["hcl"][1..].chars().all(|c| c.is_ascii_hexdigit()))
