...
fn part_1(instructions: Vec<(&str, i16, &str, fn(&i16, &i16) -> bool, i16)>) -> i16 {
    *solver(instructions).0.values().max().unwrap()
}

fn part_2(instructions: Vec<(&str, i16, &str, fn(&i16, &i16) -> bool, i16)>) -> i16 {
    solver(instructions).1
}

fn solver<'a>(
    instructions: Vec<(&'a str, i16, &'a str, fn(&i16, &i16) -> bool, i16)>,
) -> (HashMap<&'a str, i16>, i16) {
    instructions.iter().fold(
        (HashMap::new(), 0),
        |(mut acc, mut m), (ra, na, rb, f, nb)| {
