...
def solver(instructions: list[list[str]]) -> tuple[int, int]:
    ...
    """
    return get_frequency_when_recovered(instructions), count_sent_values(instructions)
...
def get_frequency_when_recovered(instructions: list[list[str]]) -> int:
    ...
    while instructions[i][0] != "rcv":
        ...
        match d[0]:
            case "snd":
                sound_frequency = reg.get_value(d[1])
            case "set":
                reg[d[1]] = reg.get_value(d[2])
            case "add":
                reg[d[1]] += reg.get_value(d[2])
            case "mul":
                reg[d[1]] *= reg.get_value(d[2])
...
def count_sent_values(instructions: list[list[str]]) -> int:
    ...
    while 1:
        ...
        match d[0]:
            case "snd":
                cnt += p
                queue[1 - p].insert(0, reg[p].get_value(d[1]))
            case "set":
                reg[p][d[1]] = reg[p].get_value(d[2])
            case "add":
                reg[p][d[1]] += reg[p].get_value(d[2])
            case "mul":
                reg[p][d[1]] *= reg[p].get_value(d[2])
            ...
            case "rcv":
                ...
                if not queue[p]:
                    ...
                    reg[p][d[1]] = queue[p].pop()
            case "jgz":
                if reg[p].get_value(d[1]) > 0:
                    i[p] += reg[p].get_value(d[2]) - 1
        i[p] += 1
    return cnt
