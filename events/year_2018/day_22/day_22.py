...
def bfs(maze: list[list[int]], start: tuple[int, int], end: tuple[int, int]):
    """
    Find shortest path in a maze with tool switching mechanics using BFS.

    Args:
        maze: 2D list representing terrain types (0=rocky, 1=wet, 2=narrow)
        start: Tuple (x, y) starting coordinates
        end: Tuple (x, y) target coordinates

    Returns:
        int: Minimum time to reach end with correct tool (torch=2)

    Note:
        - Tools: 0=neither, 1=climbing gear, 2=torch
        - Tool switching takes 7 time units
        - Movement between compatible terrains takes 1 time unit
        - Must have torch at end position
