...
fn preprocessing(input: &str) -> Vec<(usize, usize, usize)> {
    let n_bots = input.lines().filter(|l| !l.contains("value")).count();
...
fn solver(instructions: Vec<(usize, usize, usize)>) -> (usize, usize) {
    let n_bots = instructions.iter().filter(|(a, _, _)| *a != MAX).count();

    let n_outputs = instructions
        .iter()
        .filter(|(_, b, c)| *b > n_bots || *c > n_bots)
        .count();

    let mut bots: Vec<Vec<usize>> = (0..=1 + n_bots + n_outputs)
        .map(|_| Vec::new())
        .collect_vec();
    ...
    loop {
        ...
        if to_distribute.len() == 0 {
            break;
        }

        to_distribute.iter().for_each(|(n, b)| {
            if *b == [&17, &61] {
                answer = *n;
            }
