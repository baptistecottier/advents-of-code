...
fn winnings(bets: Vec<(String, usize)>, joker: char) -> usize {
    ...
    .map(|(cards, bet)| (
        ...
        cards.chars().filter(|c| *c != joker).counts().into_values().sorted().rev().collect_vec(), 
        cards.chars().map(|c| format!("{} {}", joker, "23456789TJQKA".replace(joker, "")).chars().position(|cc| cc == c).unwrap()).collect_vec(),
        bet))
