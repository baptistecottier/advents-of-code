...
fn hash(s: &str) -> u32 {
    s.chars().fold(0, |acc, c| 17 * (acc + (c as u32)) % 256)
}
    ...
    instructions
        ...
        .map(|s| {
            if s.contains('-') {
                ...
                (
                    s[..s.len() - 2].to_string(),
                    s.chars().next_back().unwrap() as u8,
                )
            }
        })
        .map(|(label, focal)| (hash(label.as_str()), label, focal))
        .fold(
            HashMap::<u32, HashMap<String, u8>>::new(),
            |lenses, (h, label, focal)| {
                if focal == 0 {
