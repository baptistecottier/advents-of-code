...
def solver(wires: dict[str, int], connections: list[tuple[str, str, str, Callable]]
           ) -> tuple[int, str]:
    """
    Solves the given wiring and connection configuration, returning the computed result and a
    manual debug output.
    """
    return (compute(wires, connections),
            manual_debug(wires, connections, explanations=False))


def compute(wires: dict[str, int], connections: list[tuple[str, str, str, Callable]]) -> int:
    """
    Computes the integer value of concatenated binary outputs for wires starting with 'z' after
    resolving all wire connections.
    """
    while None in wires.values():
        for a, b, c, f in connections:
            if all(wires[w] is not None for w in (a, b)):
                wires[c] = f(wires, a, b)
...
def manual_debug(wires: dict[str, int],
                 connections: list[tuple[str, str, str, Callable]],
                 explanations: bool) -> str:
    """
...
def swap_connections(connections: list[tuple[str, str, str, Callable]], swap1: str, swap2: str
                     ) -> list[tuple[str, str, str, Callable]]:
    """
    Swaps occurrences of swap1 and swap2 in the third element of each tuple in the connections list.
    """
    new_connections = []
    for (a, b, c, f) in connections:
        if c == swap1:
...
def display_x_plus_y_and_z(
        wires: dict[str, int],
        connections: list[tuple[str, str, str, Callable]],
        randxy: bool = False
        ) -> None:
    """
    Displays the binary sum of x and y wires and the computed value of z from the given wires and
    connections.
    """
    x, y = "", ""
